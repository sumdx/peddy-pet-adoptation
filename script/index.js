...
// AdoptModal Countdown
const adoptModal = (id) => {
  const modal = document.getElementById("adopt-modal");
  // modal.showModal()
  modal.classList.add("modal-open");
  const modalCountDown = document.getElementById("modal-countdown");
  const modalOpenButton = document.getElementById(`btn-${id}`);

  let sum = 3;
  modalCountDown.innerText = sum;
  const clockId = setInterval(() => {
    if (sum == 1) {
      modalOpenButton.innerText = "Adopted";
...
const sortPost = ()=>{
    ...
    if(currentCategories=="all"){
        ...
        const loadAllPostForSort = async () => {
            try {
              const res = await fetch(
                "https://openapi.programming-hero.com/api/peddy/pets"
              );
        ...
        const loadAllPostForSort = async () => {
            try {
              const res = await fetch(
                `https://openapi.programming-hero.com/api/peddy/category/${currentCategories}`
              );
              const data = await res.json();
              dataset=data.data;
              dataset.sort((a,b) => b.price - a.price);
...
const displayAllPost = (pets) => {
  ...
  const spinnerHide = document.getElementById("shop-div")
  spinnerHide.classList.add("hidden");
  const spinner = document.getElementById("spinner");
  spinner.classList.remove("hidden")
  const clockIdTT = setInterval(() => {
    spinner.classList.add("hidden");
    spinnerHide.classList.remove("hidden");
    clearTimeout(clockIdTT);
  }, 2000);
